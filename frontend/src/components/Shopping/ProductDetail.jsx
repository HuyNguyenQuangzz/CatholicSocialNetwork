...
const ProductDetail = () => {
  ...
  //     setProducts([]);
  //     try {
  //       const response = await fetch(`/api/productDetail/${_id}`);
  //       const data = await response.json();
  //       if (data.error) {
  //         showToast("Error", data.error, "error");
  //         return;
  ...
  // }, [_id, showToast, setProducts]); // Trigger the effect whenever the ID parameter changes

  if (!product) {
    return <Text>Loading...</Text>; // Display a loading message while fetching data
  }
